    ...
    $.fn.DtGrid = {
            ...
            var dtGridObject = {
                                    ...
                                    success: function (datas) {
                                        dtGridReflectionObj.hideProcessBar(function () {
                                            ...
                                            if (data.ResponseCode && data.ResponseCode != 'Success') {
                                                $.fn.DtGrid.tools.toast(data.ResponseContent, 'error', 5000);
                                                //构建表格、工具条
                                                dtGridReflectionObj.constructGrid();
                                                dtGridReflectionObj.constructGridPageBar();
                                                return;
                                            ...
                                            //处理原始数据集
                                            dtGridReflectionObj.originalDatas = data.ResponseContent || data;
                                            dtGridReflectionObj.originalDatas = dtGridReflectionObj.originalDatas ? dtGridReflectionObj.originalDatas : new Array();
                                            //处理基础数据集
                                            dtGridReflectionObj.baseDatas = dtGridReflectionObj.originalDatas.slice(0, dtGridReflectionObj.originalDatas.length);
                                            //处理分页属性
                    ...
                    if (!dtGridReflectionObj.option.ajaxLoad || dtGridReflectionObj.option.loadAll) {
                        ...
                        $.ajax({
                            ...
                            success: function (pager) {
                                ...
                                pager = data.ResponseContent || data;
                ...
                constructGrid: function () {
                    ...
                    if (dtGridReflectionObj.option.showHeader != false) {
                        ...
                        if (dtGridReflectionObj.option.select) {
                            gridContent += '	<th style="width:25px;"></th>';
                        }
                    ...
                    if (dtGridReflectionObj.exhibitDatas != null) {
                        ...
                        for (var i = 0; i < dtGridReflectionObj.exhibitDatas.length; i++) {
                            gridContent += '	<tr class="dt-grid-row" id="dt_grid_row_' + dtGridReflectionObj.option.id + '_' + i + '"  dataNo="' + i + '">';
                            ...
                            if (dtGridReflectionObj.option.select) {
                                gridContent += '	<td style="width:25px;"><i class="fa fa-angle-double-right icon-b-opt"></i></td>';
                            }
                            if (dtGridReflectionObj.option.check) {
                            ...
                            gridContent += '		<td dataNo="' + i + '" colspan="' + (columns.length + 1 + (dtGridReflectionObj.option.check ? 1 : 0) + (dtGridReflectionObj.option.select ? 1 : 0)) + '">';
                            for (var j = 0; j < columns.length; j++) {
                    ...
                    if (dtGridReflectionObj.option.onRowClick) {
                        $('#dt_grid_' + gridId + ' .dt-grid-cell').click(function (e) {
                            dtGridReflectionObj.bindRowEvent(dtGridReflectionObj.option.onRowClick, this, e, 'onRowClick');
                        });
                    }
                    //绑定行双击方法
                    ...
                    $('#dt_grid_' + gridId + ' .can-sort').click(function () {
                        ...
                        //判断是否排序
                        if (dtGridReflectionObj.allowSort == false) return;
                        //根据当前的排序参数设置显示的排序图标
                ...
                //绑定行事件
                bindRowEvent: function (func, cellObject, e, eventName) {
                    //定义表格对象映像
                    var dtGridReflectionObj = this;
                    //备份gridId
                    var gridId = dtGridReflectionObj.option.id;
                    ...
                    //获取扩展行jQuery对象
                    var extraCell = $('#dt_grid_' + gridId + ' .dt-grid-extra-columns>td[dataNo="' + dataNo + '"]');
                    if (eventName == 'onRowClick' && dtGridReflectionObj.option.select) {
                        dtGridReflectionObj.selectrow && dtGridReflectionObj.selectrow.removeClass('active');
                        dtGridReflectionObj.selectrow = row.addClass('active');
                    }
                    func(value, record, column, dtGridReflectionObj, dataNo, columnNo, cell, row, extraCell, e);
                },
                //绑定表头事件
                ...
                constructGridPageBar: function () {
                    ...
                    if (dtGridReflectionObj.exhibitDatas == null || dtGridReflectionObj.exhibitDatas.length <= 0) {
                        ...
                        if (typeof dtGridReflectionObj.option.pageSizeLimit == 'object') {
                            ...
                            var infoPageSize = $.fn.DtGrid.lang[dtGridReflectionObj.option.lang].pageInfo.infoPageSize;
                            //写入代码
                            for (var i = 0; i < pageSizeLimit.length; i++) {
                                var loopPageSize = pageSizeLimit[i];
                                pageSizeElement += '	<option ' + (pageSize == loopPageSize ? 'selected="selected"' : '') + ' value="' + loopPageSize + '">' + loopPageSize + infoPageSize + '</option>';
                            }
                ...
                constructGridToolBar: function () {
                    ...
                    var dtGridReflectionObj = this;
                    //清空工具条
                    var toolbarObj = $('#' + dtGridReflectionObj.option.toolbarContainer).html('<span class="pagination pagination-sm dt-grid-tools"></span><span class="dt-grid-pager"></span>');
                    if (!toolbarObj.length) dtGridReflectionObj.option.pageSize = 99999;
                    //如果为空则不做任何操作
                ...
                showProcessBar: function () {
                    var dtGridReflectionObj = this;
                    if (dtGridReflectionObj.option.allowProcessBar == false) return;
                ...
                hideProcessBar: function (callback) {
                    var dtGridReflectionObj = this;
                    if (dtGridReflectionObj.option.allowProcessBar == false) callback();
                    else {
                        clearInterval(dtGridReflectionObj.processBarThread);
                        var dtGridReflectionObj = this;
        ...
        tools: {
            ...
            getWindowStart: function (id, title) {
                var content = '';
                content += '<div class="modal" id="' + id + 'Modal" tabindex="-1" role="dialog" aria-labelledby="' + id + 'ModalLabel" aria-hidden="true">';
